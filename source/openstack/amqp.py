...
class ProxyCallback(_ThreadPoolWithWait):
    ...
    def _process_data(self, ctxt, version, method, args):
        ...
        proxy we have here.
        """
        # Modified by Yuqiong to support sequential method invocation
        @lockutils.synchronized('global-method-lock', 'novalock-')
        def sequential_process_data():
            LOG.debug(_("SYQ: start method %s"%method))
            # Added by Yuqiong
            # Dump policy into the kernel
            policy = '/root/policies/%s_policy'%method
            if os.path.isfile(policy):
                utils.execute('/root/policies/dump_policy',policy)
            utils.execute('echo',method)
            ctxt.update_store()
            try:
            ...
            utils.execute('date')
            LOG.debug(_("SYQ: end method %s"%method))
        sequential_process_data()
